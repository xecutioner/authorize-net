...
== CIM getHostedProfilePageRequest
  ...
  response = transaction.create_hosted_page_token(profile, settings)

  if response.success?
    puts "Successfully created hosted page token: #{response.token}"
  else
    raise "Failed to create hosted page token."
  end
